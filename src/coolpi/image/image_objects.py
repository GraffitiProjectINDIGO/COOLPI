...
from coolpi.auxiliary.errors import CIEIlluminantError, ClassMethodError, ColourCheckerError, FileImageError, PatchError, PathError
...
class Image(ABC):
    ...
    Methods:
        ...
        .extract_colourchecker_rgb_patches(checker_name, corners_image, parameters_draw)
    ...
    def extract_colourchecker_rgb_patches(self, checker_name, corners, size_rect):
        '''
        Method to extract the RGB data of a colour checker from an image.

        Parameters:
            checker_name     str    Colour checker name.
            corners          dict   Colour checker corner coordinaes as {"TopLeft":[x,y], "TopRight":[x,y],"BottomRight":[x.y], "BottomLeft":[x,y]}.
            size_rect        int    Patch size in pz.

        '''

        if pte.is_colourchecker_implemented(checker_name):
            checker_coordinates, rgb_patches = pte.patch_extraction(self.rgb_data, checker_name, corners, patches_coordinates=None, size_rect=size_rect)
    ...
    def show_colourchecker(self, checker_name, parameters_draw=None, show_image = True, method ="OpenCV", save_image = False, output_path = None, bits=16):
        ...
        parameters_draw = {"size_rect":self.patch_size[checker_name]["size_rect"]} if parameters_draw==None else parameters_draw # update patch size
...
class ProcessedImage(Image):
    ...
    Methods:
        ...
        .get_patch_from_colourchecker(colourchecker_name, patch_id)
        .extract_rgb_patch_data_from_image(center, size)
        .extract_colourchecker_rgb_patches(colourchecker_name, corners, parameters_draw)
        .get_ColourCheckerRGB(checker_name)
    ...
    def __init__(self, path_img=None, rgb_data=None, metadata={}):
        ...
        if self.path!=None:
            if os.path.exists(self.path):
                ...
                raise PathError(f"Invalid image path: {path_img}")
        else:
    ...
    # automatic colourchecker detection
    def automatic_colourchecker_extraction(self, checker_name="XRCCPP_24", opencv_descriptor="SIFT"):  
        bits_image = rwo.get_bits_image(self.rgb_data)
        image_rgb = self.rgb_data/(math.pow(2,bits_image)-1) if bits_image>0 else self.rgb_data # range [0,1]
        image_rgb = np.clip(image_rgb*(math.pow(2,8)-1), 0, (math.pow(2, 8)-1)) # 8bits
        image_rgb = np.uint8(image_rgb) # format conversion to avoid error using openCV 
        corners = ccd.colourchecker_patch_detection(image_rgb, opencv_descriptor, checker_name)   
        if corners is not None:
            has_colourchecker = True          
            size_rect = ccd.compute_size_rect(corners, checker_name)                    
            self.extract_colourchecker_rgb_patches(checker_name, corners, size_rect)
            return has_colourchecker, corners, size_rect
...
class RawImage(Image):
    ...
    Methods:
        ...
        .automatic_colourchecker_extraction(checker_name, opencv_descriptor="SIFT")
    ...
    def automatic_image_processing(self, show_image = True, save_image = False, output_path = None, method="OpenCV", **krawargs):
        ...
        if show_image:
            ...
            return ProcessedImage(path_img=output_path, rgb_data=rgb_processed, metadata=self.metadata)

        if save_image:
            rwo.save_rgb_array_as_image(rgb_processed, output_path, bits=rwo.get_bits_image(rgb_processed))
            return ProcessedImage(path_img=output_path, metadata=self.metadata)
        
        return ProcessedImage(path_img=output_path, rgb_data=rgb_processed, metadata=self.metadata) # rgb_processed 
    ...
    def compute_wb_multipliers(self, **params):
        ...
        if "colourchecker_name" and "patch_id" in params.keys():
            wb_multipliers = self.__compute_white_balance_multipliers_from_grey_patch__(params["checker_name"], params["patch_id"])
    ...
    def __estimate_white_balance_multipliers_using_wb_algorithm__(self, algorithm="GreyWorld", remove_colourckecker=False, corners_colourchecker=None):
        algorithm = "GreyWorld" if algorithm==None else algorithm
        rgb_data = wb.mask_colourchecker(self.rgb_data, corners_colourchecker, -1) if remove_colourckecker else self.rgb_data
        estimated_wb_multipliers = wb.estimate_white_balance_multipliers(rgb_data, algorithm)
    ...
    # automatic colourchecker detection
    def automatic_colourchecker_extraction(self, checker_name, opencv_descriptor="SIFT"):
        rgb_processed = rwp.automatic_raw_image_processing(self.path)  # 8bits
        corners = ccd.colourchecker_patch_detection(rgb_processed, opencv_descriptor, checker_name)   
        if corners is not None:
            has_colourchecker = True          
            size_rect = ccd.compute_size_rect(corners, checker_name)                    
            self.extract_colourchecker_rgb_patches(checker_name, corners, size_rect)
