...
from coolpi.auxiliary.errors import ClassMethodError
...
def mask_colourchecker(rgb_data, corners_colourchecker, mask_value=-1):
    ...
    rgb_mask[int(y1):int(y2), int(x1):int(x2), :] = mask_value
...
def estimate_white_balance_multipliers(rgb_data, algorithm="GreyWorld"):
    if algorithm=="Average":
        ...
        raise ClassMethodError("Algorithm not implemented")
    return estimated_wb_multipliers

def compute_wb_average(wb_factors):
    r_gain = [wb[0] for wb in wb_factors]
    g_gain = [wb[1] for wb in wb_factors]
    b_gain = [wb[2] for wb in wb_factors]
    g2_gain = [wb[3] for wb in wb_factors]
    return [np.average(r_gain), np.average(g_gain), np.average(b_gain), np.average(g2_gain)]
