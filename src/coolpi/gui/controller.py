...
class Controller:
    ...
    def _connect_cde_zoom(self):
        ...
        self._view._cde._save_button.clicked.connect(self.save_plot)
    ...
    def _connect_spd_items(self):
        self.connect_spd_radio_buttons()
    ...
    def connect_spd_radio_buttons(self):
        self._view._spd._spd_input._cie_radio_button.toggled.connect(lambda: self.spd_radio_button_checked(self._view._spd._spd_input._cie_radio_button)) 
        self._view._spd._spd_input._cct_radio_button.toggled.connect(lambda: self.spd_radio_button_checked(self._view._spd._spd_input._cct_radio_button))
        self._view._spd._spd_input._user_radio_button.toggled.connect(lambda: self.spd_radio_button_checked(self._view._spd._spd_input._user_radio_button))

    def spd_radio_button_checked(self, button):
        self.close_open_figure_window()
        self.clear_labels(self._view._spd)
        if button.text() == "CIE":
            self._view._spd._spd_input._combo_illuminant.setEnabled(True)
            self._view._spd._spd_input._cct_edit.setEnabled(False)
            self._view._spd._spd_input._user_name_edit.setEnabled(False)
            self._view._spd._spd_input.desactivate_load_button()
        elif button.text() == "CCT":
            self._view._spd._spd_input._combo_illuminant.setCurrentText("D65") # default
            self._view._spd._spd_input._combo_illuminant.setEnabled(False)
            self._view._spd._spd_input._cct_edit.setEnabled(True)
            self._view._spd._spd_input._user_name_edit.setEnabled(False)
            self._view._spd._spd_input.desactivate_load_button()
        elif button.text() == "Measured":
            self._view._spd._spd_input._combo_illuminant.setCurrentText("D65")
            self._view._spd._spd_input._combo_illuminant.setEnabled(False)
            self._view._spd._spd_input._cct_edit.setEnabled(False)
            self._view._spd._spd_input.activate_load_button()
            self._view._spd._spd_input._user_name_edit.setEnabled(True)
    ...
    def close_open_windows(self):
        windows_to_close = ["_csc","_cde", "_cpt", "_spc", "_spd", "_cci", "_rcip", "_figure"]
